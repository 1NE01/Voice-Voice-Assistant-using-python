...
def speak(text):
    ...
    filename="sound.mp3"
    tts.save(filename)
    playsound.playsound('C:\\Code\\Python\\voice assitant\\sound.mp3')
'''
...
def click():# This click function is use to click the send message button in whatapps web


    time.sleep(1) # Line - Use to delay the second to load website .
    locatepic=pg.locateOnScreen('image/2.png')
    pg.click(locatepic)
...
def lisa():
    ...
    if "open youtube" in query:
        ...
        if 'want to send a message' in query:
             whatapp()
        else:
